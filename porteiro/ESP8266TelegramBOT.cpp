...
/*
  ...
  Ref. Library at https:github/esp8266/Arduino
	...
	Modified version by Pedro Caetano.
...
void TelegramBOT::configuraBOT(String token, String name, String username)	{
	_token=token;
	_name=name;
  _username=username;
}
...
void TelegramBOT::getUpdates(String offset)  {
    ...
    String mess=connectToTelegram(command);       //recieve reply from telegram.org
		...
		JsonObject& jsonMessage = jsonBuffer.parseObject(mess);
...
void TelegramBOT::analizeMessages(void)     {
  ...
  for (int i=1; i<message[0][0].toInt()+1; i++)      {
    ...
    //MODIFICAÇÃO: message[i][6] guarda o nome de usuário
    DynamicJsonBuffer jsonBuffer;
    JsonObject& jsonMessage = jsonBuffer.parseObject(a);
    jsonMessage["message"]["from"]["username"].printTo(message[i][6]);
    message[i][5]="";
    for (int m=5; m<messageLenght+1; m++)             {
